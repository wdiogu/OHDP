...
network_calc_tool = _MODELLER.tool(
...
matrix_calc_tool = _MODELLER.tool(
...
class AssignTransit(_m.Tool()):
    ...
    def _execute(self, scenario, parameters):
        load_input_matrix_list = self._load_input_matrices(
            parameters, "demand_matrix")
    ...
    def _load_output_matrices(self, parameters, matrix_name=list):
        ...
        for i in range(0, len(matrix_name)):
            mtx_dict[matrix_name[i]] = [tc[matrix_name[i]]
            ...
            for mtx in load_output_matrix_dict[matrix_name]:
                if mtx != None:
                    ...
                    if matrix_name == "impedance_matrix":
                        _write(
                            'Creating Temporary Impedance Matrix "%s"', matrix_name)
                        matrix = _util.initialize_matrix(
                            default=0.0,
                            description=description if description != "" else desc,
                            matrix_type="FULL",
    ...
    def _change_walk_speed_4p0(self, scenario, walk_speed):
        change_mode_tool = _MODELLER.tool(
            "inro.emme.data.network.mode.change_mode")
    ...
    def _create_walk_time_peception_attribute_list(
        ...
        for tc_parameter in parameters["transit_classes"]:
            ...
            walk_time_peception_attribute_list.append(
                walk_time_peception_attribute)
    ...
    def _assign_walk_perception(self, scenario, perceptionArray, WalkAttributeId):
        exatt = scenario.extra_attribute(WalkAttributeId)
        ...
        try:
            ...
            for temp_attribute in temp_attribute_list:
                if temp_attribute is not None:
                    ...
                    _write("Deleted temporary '%s' link attribute" %
                           temp_attribute.id)
